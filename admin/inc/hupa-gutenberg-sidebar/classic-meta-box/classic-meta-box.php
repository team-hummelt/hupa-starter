                ...
                <div class="card-body">
                    ...
                    <div class="col-xl-4 col-lg-6 col-sm-8 col-12 mb-3">
                        <label for="selectTopAreaMenu"
                               class="form-label"><?= __( 'Top Area menu', 'bootscore' ) ?></label>
                        <select class="form-select" name="topAreaSelect" id="selectTopAreaMenu">
							<?php foreach ( apply_filters( 'get_settings_menu_label', 'topArea' ) as $tmp ):
								$topAreaSelect == $tmp['value'] ? $sel = 'selected' : $sel = '';
								?>
                                <option value="<?= $tmp['value'] ?>" <?= $sel ?>><?= $tmp['label'] ?></option>
							<?php endforeach; ?>
                        </select>
